...
public class MemoryGame {
    ...
    public String generateRandomString(int n) {
        ...
        for (int i = 0; i < n; i++) {
            randomString += CHARACTERS[Math.abs(rand.nextInt()) % 26];
        }
        return randomString;
    }
    ...
    public void flashSequence(String letters) {
        ...
        for (int i = 0; i < letters.length(); i++) {
            StdDraw.pause(500);
            String temp = Character.toString(letters.charAt(i));
            drawFrame(temp);
            StdDraw.pause(1000);
        }
    }
...
//        }
        while (true) {
            ...
            StdDraw.pause(500);
            String systemProduct = generateRandomString(round);
            flashSequence(systemProduct);
            String usrInput = solicitNCharsInput(round);
            if (systemProduct.equals(usrInput)) {
                round += 1;
            } else {
